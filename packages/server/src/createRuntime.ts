...
import { ContentService, QuizService } from '@hfroemmel/quiz-core'
...
export interface ComposedQuizRuntime {
  service: QuizService
  store: QuizStore
 ...
 */
export function createQuizRuntime(options: QuizRuntimeOptions): ComposedQuizRuntime {
  const { packageDir, databaseFile } = options

  const store = new QuizStore(databaseFile)
  // Rueckfallwurzeln ohne Assets-Verzeichnis fallen hier heraus - der Kern
  // fasst kein Dateisystem an und bekommt nur brauchbare Wurzeln.
  const mediaFallbackDirs = (options.mediaFallbackDirs ?? []).filter((dir) => existsSync(join(dir, 'assets')))
  const content = new ContentService(loadQuizPackage(packageDir), [], mediaFallbackDirs)
